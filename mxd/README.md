...
After the terraform command has successfully completed minio console will look like below.

![img.png](assets/dashboard.png)

![img.png](assets/alice_bucket.png)

Create asset metadata for provider connector i.e Alice.

#### 8.1 Create Asset
Create an asset using the following `curl` command:
```shell
curl --location 'http://localhost/alice/management/v2/assets' \
--header 'Content-Type: application/json' \
--header 'X-Api-Key: password' \
--data-raw '{
    "@context": {},
...
```
Alice defines an access policy which allows only Bob (with BPN Number BPNL000000000002) to view its catalog.

#### 8.3 Create Contract Definition
Create a contract definition using the following `curl` command:
```shell
...
```
#### 8.4 Initiate Negotiation

Now Bob will try to initiate Negotiation
...
```
Negotiation status can be fetched using below curl command
```shell
curl --location --request POST 'http://localhost/bob/management/v2/contractnegotiations/request' \
--header 'X-Api-Key: password'
```
...
[
    {
        ...
        "@id": "71b73000-49ff-42dc-967c-12cfe4887905",
        "edc:type": "CONSUMER",
        ...
        "edc:createdAt": 1698915159074,
        "edc:contractAgreementId": "568ef5e2-f43f-4cc8-8189-c6f544d27f35",
        "@context": {
            "dct": "https://purl.org/dc/terms/",
            "tx": "https://w3id.org/tractusx/v0.0.1/ns/",
            "edc": "https://w3id.org/edc/v0.0.1/ns/",
...
```
#### 8.4 Initiate Transfer

Now after negotiation finalized now below curl command is used for transfer process
...
--data-raw '{
    ...
    "contractId": "568ef5e2-f43f-4cc8-8189-c6f544d27f35",
    "dataDestination": {
        "type": "AmazonS3",
        "keyName":"document.txt",
...
```
Now we will able to see the transfered asset in bob bucket

![img.png](assets/bob_bucket.png)

## 9. Improving the setup
